...
public class TheQueen extends EntityMob {
	...
	public void onUpdate() {
		...
		if (this.player_hit_count < 10 && this.getHealth() < (float)(this.mygetMaxHealth() * 3 / 4)) {
			this.attdam = (double)(OreSpawnMain.TheQueen_stats.attack * 20);
		}

		if (this.player_hit_count < 10 && this.getHealth() < (float)(this.mygetMaxHealth() / 2)) {
			this.attdam = (double)(OreSpawnMain.TheQueen_stats.attack * 100);
		}

		if (this.player_hit_count < 10 && this.getHealth() < (float)(this.mygetMaxHealth() / 3)) {
			this.attdam = (double)(OreSpawnMain.TheQueen_stats.attack * 500);
		}

		if (this.player_hit_count < 10 && this.getHealth() < (float)(this.mygetMaxHealth() / 4)) {
			this.attdam = (double)(OreSpawnMain.TheQueen_stats.attack * 1000);
		}
	...
	protected void updateAITasks() {
		...
		if (!this.isDead) {
			super.updateAITasks();
			if (this.ev != null) {
				if (this.getDistanceSqToEntity(this.ev) < (double)2000.0F && !this.ev.isDead) {
			...
			if (this.attack_level > 1000) {
				if (this.mood == 1) {
					...
					if (this.worldObj.getGameRules().getGameRuleBooleanValue("mobGriefing")) {
						for (int m = 0; m < 25; ++m) {
							int i = this.worldObj.rand.nextInt(25) - this.worldObj.rand.nextInt(25);
			...
			if (!this.tooFarFromHome() && this.worldObj.rand.nextInt(200) != 0 && !(this.currentFlightTarget.getDistanceSquared((int)this.posX, (int)this.posY, (int)this.posZ) < 9.1F)) {
				if (this.worldObj.rand.nextInt(attrand) == 0) {
					e = this.rt;
					...
					if (this.head_found == 0 && this.mood == 1) {
						EntityLiving var10000 = (EntityLiving)spawnCreature(this.worldObj, "QueenHead", this.posX, this.posY + 20.0D, this.posZ);
					}
					...
					if (e != null) {
						...
						this.setAttacking(1);
						if (this.backoff_timer == 0) {
							int dist = (int)(e.posY + (double)(e.height / 2.0F) + 1.0D);
							if (dist > 230) {
								dist = 230;
							}
						...
						if (this.getDistanceSqToEntity(e) < (double)900.0F) {
							if (this.worldObj.rand.nextInt(2) == 1) {
								this.doJumpDamage(this.posX, this.posY, this.posZ, 15.0D, (double)(OreSpawnMain.TheQueen_stats.attack / 4), 0);
							}
						...
						if (this.worldObj.rand.nextInt(3) == 1) {
							this.doJumpDamage(dx, this.posY + 10.0D, dz, 15.0D, (double)(OreSpawnMain.TheQueen_stats.attack / 2), 1);
						}

						if (this.getHorizontalDistanceSqToEntity(e) > (double)900.0F) {
				...
				if (this.mood == 0) {
					...
					if (kinglist != null) {
						...
						if (var2.hasNext()) {
							TheKing var100 = null;
							TheKing var101 = (TheKing)var2.next();
							this.guard_mode = 0;
							...
							this.currentFlightTarget.set((int)var101.posX + xdir, (int)(var101.posY + (double)(this.worldObj.rand.nextInt(8) - this.worldObj.rand.nextInt(8))), (int)var101.posZ + zdir);
						}
					}
				}
			}
	...
	public int getTotalArmorValue() {
		...
		} else if (this.player_hit_count < 10 && this.getHealth() < (float)(this.mygetMaxHealth() / 2)) {
			return OreSpawnMain.TheQueen_stats.defense + 3;
		} else {
			return this.player_hit_count < 10 && this.getHealth() < (float)(this.mygetMaxHealth() / 3) ? OreSpawnMain.TheQueen_stats.defense + 5 : OreSpawnMain.TheQueen_stats.defense;
		}
	}
	...
	private boolean isSuitableTarget(EntityLivingBase par1EntityLiving, boolean par2) {
		if (par1EntityLiving == null) {
			...
			return false;
		} else if (par1EntityLiving instanceof QueenHead) {
			this.head_found = 1;
			return false;
