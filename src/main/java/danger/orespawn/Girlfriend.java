...
public class Girlfriend extends EntityTameable implements IRangedAttackMob
{
	...
	public Girlfriend(World par1World)
	{
		...
		if (OreSpawnMain.valentines_day != 0) this.setSize(2.5F, 8.0F);
		...
		this.tasks.addTask(2, new EntityAITempt(this, 1.25D, Item.getItemFromBlock(Blocks.red_flower), false));
		this.Dance = new MyEntityAIDance(this);
		this.tasks.addTask(3, this.Dance);
		this.tasks.addTask(4, new EntityAIArrowAttack(this, 1.25D, 20, 10.0F));
		...
		if (OreSpawnMain.PlayNicely == 0) this.targetTasks.addTask(2, new MyEntityAINearestAttackableTarget(this, EntityCreeper.class, 20.0F, 0, true, true, IMob.mobSelector));

		if (OreSpawnMain.PlayNicely == 0) this.targetTasks.addTask(3, new MyEntityAINearestAttackableTarget(this, EntityLiving.class, 15.0F, 0, true, true, IMob.mobSelector));

		if (OreSpawnMain.PlayNicely == 0) this.targetTasks.addTask(4, new MyEntityAIJealousy(this, Girlfriend.class, 6.0F, 5, true));
		if (OreSpawnMain.PlayNicely == 0) this.targetTasks.addTask(5, new MyEntityAIJealousy(this, Girlfriend.class, 3.0F, 15, true));
			...
			if (e != null && e instanceof EntityPlayer) {
				...
				if (r != null) {
					if (r instanceof Elevator) {
						...
						float f = -0.45F;
						this.setPosition(r.posX - (double)f * Math.sin(Math.toRadians((double)r.rotationYaw)), r.posY, r.posZ + (double)f * Math.cos(Math.toRadians((double)r.rotationYaw)));
	...
	public void readEntityFromNBT(NBTTagCompound par1NBTTagCompound)
	{
		...
		if (OreSpawnMain.valentines_day != 0 && this.feelingBetter != 0) this.setSize(0.5F, 1.6F);
	}
	...
	protected void updateAITick()
	{
		...
		if (OreSpawnMain.PlayNicely != 0) victim = null;
		if (this.worldObj.rand.nextInt(100) == 1) this.setRevengeTarget(null);
		if (this.worldObj.rand.nextInt(200) == 1) this.setAttackTarget(null);
		...
		if (stack != null && !this.isSitting())
		{
					...
					if (this.getHeldItem() != null)
					{
						if (this.getDistanceToEntity(victim) < 4.0F || stack.getItem() == OreSpawnMain.MyBertha && this.getDistanceToEntity(victim) < 10.0F)
						{
							...
							if (this.attackTime <= 0)
							{
								...
								{
									if (!this.worldObj.isRemote && this.voice_enable != 0) this.worldObj.playSoundAtEntity(this, "orespawn:o_fight", 0.5F, this.getSoundPitch());	
						...
						else if (this.getDistanceToEntity(victim) < 7.0F && stack.getItem() != OreSpawnMain.MyUltimateBow)
						{
							...
							{
								if (!this.worldObj.isRemote && this.voice_enable != 0) this.worldObj.playSoundAtEntity(this, "orespawn:o_taunt", 0.5F, this.getSoundPitch());
								this.taunt_sound_ticker = 300;
							}
	
							this.getNavigator().tryMoveToEntityLiving(victim, 1.25D);
						}
					}
				}
			} else
			{
				...
				if (this.had_target != 0) {
					this.had_target = 0;
					if (!this.worldObj.isRemote && this.voice_enable != 0) this.worldObj.playSoundAtEntity(this, "orespawn:o_woohoo", 0.4F, this.getSoundPitch());
				}
			}
		}
	}
	...
	{
		if (OreSpawnMain.valentines_day != 0 && this.feelingBetter == 0) return ValentineTexture;
		if (this.wet_count <= 0)
		{
			int txture = this.getTameSkin();
			
			if (this.is_princess == 1) return PrincessTexture1;
			if (this.is_princess == 2) return PrincessTexture2;

			if (txture == 0) return DryTexture0;
			if (txture == 1) return DryTexture1;
			if (txture == 2) return DryTexture2;
			if (txture == 3) return DryTexture3;
			if (txture == 4) return DryTexture4;
			if (txture == 5) return DryTexture5;
			if (txture == 6) return DryTexture6;
			if (txture == 7) return DryTexture7;
			if (txture == 8) return DryTexture8;
			if (txture == 9) return DryTexture9;
			if (txture == 10) return DryTexture10;
			if (txture == 11) return DryTexture11;
			if (txture == 12) return DryTexture12;
			if (txture == 13) return DryTexture13;
			if (txture == 14) return DryTexture14;
			if (txture == 15) return DryTexture15;
			if (txture == 16) return DryTexture16;
			if (txture == 17) return DryTexture17;
			if (txture == 18) return DryTexture18;
			if (txture == 19) return DryTexture19;
			if (txture == 20) return DryTexture20;
			if (txture == 21) return DryTexture21;
			if (txture == 22) return DryTexture22;
			if (txture == 23) return DryTexture23;
			if (txture == 24) return DryTexture24;
			if (txture == 25) return DryTexture25;
			if (txture == 26) return DryTexture26;
			if (txture == 27) return DryTexture27;
			if (txture == 28) return DryTexture28;
			if (txture == 29) return DryTexture29;
			if (txture == 30) return DryTexture30;
			if (txture == 31) return DryTexture31;
			if (txture == 32) return DryTexture32;
			if (txture == 33) return DryTexture33;
			if (txture == 34) return DryTexture34;
			if (txture == 35) return DryTexture35;
			if (txture == 36) return DryTexture36;
			if (txture == 37) return DryTexture37;
			...
			if (temp == 1) return WetTexture1;
			if (temp == 2) return WetTexture2;
			if (temp == 3) return WetTexture3;
			if (temp == 4) return WetTexture4;
			if (temp == 5) return WetTexture5;
			if (temp == 6) return WetTexture6;
			if (temp == 7) return WetTexture7;
			if (temp == 8) return WetTexture8;
			if (temp == 9) return WetTexture9;
			if (temp == 10) return WetTexture10;
			if (temp == 11) return WetTexture11;
			if (temp == 12) return WetTexture12;
			if (temp == 13) return WetTexture13;
			if (temp == 14) return WetTexture14;
			if (temp == 15) return WetTexture15;
			if (temp == 16) return WetTexture16;
			if (temp == 17) return WetTexture17;
		}
	...
	{
		if (OreSpawnMain.valentines_day != 0 && this.feelingBetter == 0) return 800;
		return 80;
	}
	...
	public boolean interact(EntityPlayer par1EntityPlayer)
	{
		...
		if (var2 != null && (var2.getItem() == Item.getItemFromBlock(Blocks.red_flower) || var2.getItem() == Item.getItemFromBlock(OreSpawnMain.CrystalFlowerRedBlock)) && par1EntityPlayer.getDistanceSqToEntity(this) < 16.0D)
		{
			if (!this.isTamed())
			{
				if (!this.worldObj.isRemote)
		...
		if (this.isTamed() && var2 != null && var2.getItem() == OreSpawnMain.MyRuby && par1EntityPlayer.getDistanceSqToEntity(this) < 16.0D && this.func_152114_e(par1EntityPlayer))
		{
		...
		if (this.isTamed() && var2 != null && var2.getItem() == OreSpawnMain.MyAmethyst && par1EntityPlayer.getDistanceSqToEntity(this) < 16.0D && this.func_152114_e(par1EntityPlayer))
		{
		...
		if (this.isTamed() && var2 != null && (var2.getItem() == Item.getItemFromBlock(Blocks.yellow_flower) || var2.getItem() == Item.getItemFromBlock(OreSpawnMain.CrystalFlowerYellowBlock)) && par1EntityPlayer.getDistanceSqToEntity(this) < 16.0D && this.func_152114_e(par1EntityPlayer))
		{
			...
			if (!this.worldObj.isRemote)
			{
				if (this.wet_count > 0 || this.isInWater() || this.handleLavaMovement()) {
					++this.which_wet_girl;
					if (this.which_wet_girl > 17) this.which_wet_girl = 0;
					this.setWetTameSkin(this.which_wet_girl);
					this.worldObj.setEntityState(this, (byte)7);
					if (this.isInWater() || this.handleLavaMovement()) {
						this.wet_count = 500;
					...
					++this.which_girl;
					if (this.which_girl > 40) this.which_girl = 0;
					this.setTameSkin(this.which_girl);
					this.worldObj.setEntityState(this, (byte)7);
				}
			}
			...
			return true;
		} else if (this.isTamed() && var2 != null && this.func_152114_e(par1EntityPlayer) && par1EntityPlayer.getDistanceSqToEntity(this) < 16.0D)
		{
				...
				} else
				{
					...
					if (itm instanceof ItemOreSpawnArmor) {
						if (itm == OreSpawnMain.CrystalPinkHelmet || itm == OreSpawnMain.TigersEyeHelmet) {
							ItemStack v4 = this.getEquipmentInSlot(4);
		...
		else if (this.isTamed() && var2 != null && var2.getItem() == Item.getItemFromBlock(Blocks.diamond_block) && par1EntityPlayer.getDistanceSqToEntity(this) < 16.0D)
		{
			this.setSitting(false);
			this.setTamed(true);
			this.func_152115_b(par1EntityPlayer.getUniqueID().toString());
		...
		else if (this.isTamed() && var2 == null && par1EntityPlayer.getDistanceSqToEntity(this) < 16.0D && this.func_152114_e(par1EntityPlayer))
		{
	...
	protected String getLivingSound()
	{
		...
		if (this.Dance.is_dancing != 0) return null;
	...
	public void attackEntityWithRangedAttack(EntityLivingBase par1EntityLiving)
	{
		...
		{
			EntityArrow var8 = new UltimateArrow(this.worldObj, this, par1EntityLiving, 2.0F, 10.0F);
			
			
			if (this.worldObj.rand.nextInt(4) == 1) var8.setIsCritical(true);
	...
	public void attackTargetEntityWithCurrentItem(Entity par1Entity)
	{
		...
		if (stack != null)
		{
			...
			{
				boolean var5 = this.fallDistance > 0.0F && !this.onGround && !this.isOnLadder() && !this.isInWater() && !this.handleLavaMovement() && !this.isPotionActive(Potion.blindness) && this.ridingEntity == null && par1Entity instanceof EntityLiving;
				...
				var2 += var4;
				boolean var6 = par1Entity.attackEntityFrom(DamageSource.causeMobDamage(this), var2);
				
				if (var6)
				{
					if (var3 > 0)
					{
						par1Entity.addVelocity((double)(-MathHelper.sin(this.rotationYaw * (float)Math.PI / 180.0F) * (float)var3 * 0.5F), 0.1, (double)(MathHelper.cos(this.rotationYaw * (float)Math.PI / 180.0F) * (float)var3 * 0.5F));
						this.motionX *= 0.6;
						this.motionZ *= 0.6;
