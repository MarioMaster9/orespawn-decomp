...
public class Island extends EntityAnimal {
	...
	public void onUpdate() {
		...
		if (!this.worldObj.isRemote) {
			...
			if (this.just_spawned != 0) {
				this.dir = this.worldObj.rand.nextFloat() * (float)Math.PI;
	...
	private void create_island() {
		...
		for (int i = 0; i < this.depth; i++) {
			...
			for (double curdir = -3.1415926; curdir < 3.1415926; curdir += deltadir) {
				double tradius = (double)this.radius;
				tradius /= (double)(i + 1);

				for (double h = 0.75D; h < tradius; h += deltamag) {
	...
	private void update_island() {
		...
		int xoff = 0;
		int zoff = 0;
		this.myX += (double)this.speed * Math.cos((double)this.dir);
		this.myZ += (double)this.speed * Math.sin((double)this.dir);
		int mx = (int)this.myX;
		int mz = (int)this.myZ;
		...
		if (mx != px || mz != pz) {
			for (int i = 0; i < this.depth; i++) {
				...
				for (double curdir = -3.3; curdir < 3.3; curdir += deltadir / 2.0D) {
					double tradius = (double)this.radius;
					tradius /= (double)(i + 1);
			...
			for (int var33 = 0; var33 < this.depth; ++var33) {
				izlast = 0;
				ixlast = 0;

				for (double curdir = -3.1415926; curdir < 3.1415926; curdir += deltadir) {
					double tradius = (double)this.radius;
					tradius /= (double)(var33 + 1);
					...
					for(; h < tradius; h += deltamag) {
						...
						if (ix != ixlast || iz != izlast) {
							...
							if (var33 == 0) {
								Block bid;
								if ((bid = this.worldObj.getBlock(ix, (int)this.posY - var33 + 1, iz)) == Blocks.air) {
									if (this.worldObj.rand.nextInt(5000) == 1) {
										this.worldObj.setBlock(ix, (int)this.posY - var33 + 1, iz, Blocks.lava);
									} else {
										this.FastSetBlock(ix, (int)this.posY - var33 + 1, iz, Blocks.mycelium);
										if (this.worldObj.rand.nextInt(20) == 1 && this.worldObj.getBlock(ix, (int)this.posY - var33 + 2, iz) == Blocks.air) {
											if (this.worldObj.rand.nextInt(2) == 1) {
												this.worldObj.setBlock(ix, (int)this.posY - var33 + 2, iz, Blocks.brown_mushroom);
											} else {
												this.worldObj.setBlock(ix, (int)this.posY - var33 + 2, iz, Blocks.red_mushroom);
											}
										}
									}
								} else if (bid == Blocks.bedrock) {
								...
								Block bid = this.worldObj.getBlock(ix, (int)this.posY - var33 + 1, iz);
								if (bid == Blocks.stone) {
									if (!this.worldObj.isRemote) {
										this.worldObj.createExplosion(this, (double)ix, this.posY - (double)var33 + 1.0D, (double)iz, 5.0F, true);
									}
								} else if (this.worldObj.rand.nextInt(10) == 1) {
									this.FastSetBlock(ix, (int)this.posY - var33 + 1, iz, Blocks.diamond_ore);
								} else {
									this.FastSetBlock(ix, (int)this.posY - var33 + 1, iz, Blocks.end_stone);
								}
							}
						}
					}
		...
		if (this.worldObj.rand.nextInt(2 + 2000 / this.timer) == 1) {
			...
			if (!var2.hasNext()) {
				EntityCreature var10000 = (EntityCreature)spawnCreature(this.worldObj, "Triffid", this.posX, this.posY + 2.01, this.posZ);
			}
		}
