...
public class ModelSeaViper extends ModelBase
{
	...
	public ModelSeaViper(float f1)
	{
		...
		this.setRotation(this.tBase, (-(float)Math.PI / 6F), 0.0F, 0.0F);
		...
		this.t3.mirror = true;
		this.setRotation(this.t3, (-(float)Math.PI / 6F), 0.0F, 0.0F);
	...
	public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5)
	{
		...
		if (e.getAttacking() != 0) {
			...
			newangle = MathHelper.cos(f2 * 4.7F * this.wingspeed) * (float)Math.PI * 0.07F;
			...
			newangle = MathHelper.cos(f2 * 1.5F * this.wingspeed) * (float)Math.PI * 0.05F;
			this.ToungBase.offsetZ = this.MiddleTounge.offsetZ = this.ForkLeft.offsetZ = this.ForkRight.offsetZ = newangle;
		} else {
			newangle = MathHelper.cos(f2 * 0.2F * this.wingspeed) * (float)Math.PI * 0.02F;
			this.MouthBottom.rotateAngleX = 0.45F + newangle;
			newangle = MathHelper.cos(f2 * 1.7F * this.wingspeed) * (float)Math.PI * 0.03F;
			this.ToungBase.rotateAngleX = 0.261F + newangle;
			this.MiddleTounge.rotateAngleX = 0.174F + newangle;
			this.ForkLeft.rotateAngleX = 0.087F + newangle;
			this.ForkRight.rotateAngleX = 0.087F + newangle;
		...
		this.Head.rotateAngleY = this.MouthTop.rotateAngleY = this.EyeLeft.rotateAngleY = this.EyeRight.rotateAngleY = newangle;
		this.FangLeft.rotateAngleY = this.FangRight.rotateAngleY = newangle;
		
		this.MouthBottom.rotateAngleY = newangle;
		this.MouthBottom.rotationPointZ = this.Head.rotationPointZ - (float)Math.cos((double)this.Head.rotateAngleY) * 2.0F;
		this.MouthBottom.rotationPointX = this.Head.rotationPointX - (float)Math.sin((double)this.Head.rotateAngleY) * 2.0F;
	...
	private void doseg(ModelRenderer inn, ModelRenderer notinn, float f, float f1, float f2)
	{
		...
		notinn.rotationPointZ = (float)((double)inn.rotationPointZ + (double)((float)Math.cos(inn.rotateAngleY)) * (9.0F * Math.abs(Math.cos(inn.rotateAngleX))));
		...
		newangle = MathHelper.cos(f2 * 1.3F * this.wingspeed - pi4 * f) * (float)Math.PI * 0.2F * f1;
