...
public class ModelSkate extends ModelBase
{
	...
	public ModelSkate(float f1)
	{
		...
		this.setRotation(this.body, 0.0F, ((float)Math.PI / 4F), 0.0F);
	...
	public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5)
	{
		...
		if ((double)f1 > 0.1) {
			newangle = MathHelper.cos(f2 * 1.2F) * (float)Math.PI * 0.15F * f1;
		} else {
			newangle = MathHelper.cos(f2 * 0.4F) * (float)Math.PI * 0.05F;
		}
