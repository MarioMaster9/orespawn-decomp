...
public class ModelCloudShark extends ModelBase
{
	...
	public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5)
	{
		...
		newangle = MathHelper.cos(f2 * 0.7F * this.wingspeed) * (float)Math.PI * 0.15F;
		this.leftfin.rotateAngleY = 1.15F + newangle;
		newangle = MathHelper.cos(f2 * 1.5F * this.wingspeed) * (float)Math.PI * 0.15F;
		...
		newangle = MathHelper.cos(f2 * 0.5F * this.wingspeed) * (float)Math.PI * 0.1F;
