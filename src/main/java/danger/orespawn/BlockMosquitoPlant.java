...
public class BlockMosquitoPlant extends BlockCrops
{
	...
	public void updateTick(World par1World, int par2, int par3, int par4, Random par5Random)
	{
		...
		rate = 7 - rate;
		if (rate > 1) {
			if (OreSpawnMain.OreSpawnRand.nextInt(rate) != 0) return;
		}
		...
		if (bid == Blocks.air)
		{
			howmany = 2 + OreSpawnMain.OreSpawnRand.nextInt(5);
			for (i = 0; i < howmany; i++) {
				spawnCreature(par1World, "Mosquito", (double)par2 + 0.5D, (double)par3 + 1.01, (double)par4 + 0.5D);
			}
		}
	}
