...
public class EntityCannonFodder extends EntityTameable {
	...
	public boolean interact(EntityPlayer par1EntityPlayer) {
		...
		if (super.interact(par1EntityPlayer)) {
			...
			if (var2 != null && var2.getItem() == Items.carrot && par1EntityPlayer.getDistanceSqToEntity(this) < 16.0D) {
				this.hat_color = 1;
				...
				return true;
			} else if (var2 != null && var2.getItem() == Items.potato && par1EntityPlayer.getDistanceSqToEntity(this) < 16.0D) {
				this.hat_color = 3;
				if (this.name_one == null) {
					this.name_one = par1EntityPlayer.getUniqueID().toString();
				}
				...
				return true;
			} else if (var2 != null && var2.getItem() == OreSpawnMain.MyQuinoa && par1EntityPlayer.getDistanceSqToEntity(this) < 16.0D) {
				this.hat_color = 2;
				if (this.name_one == null) {
					this.name_one = par1EntityPlayer.getUniqueID().toString();
				}
				...
				return true;
			} else if (var2 != null && this.is_activated == 2 && var2.getItem() == OreSpawnMain.MyCornCob && par1EntityPlayer.getDistanceSqToEntity(this) < 16.0D) {
				...
				if (!this.worldObj.isRemote) {
					Entity newent = spawnCreature(this.worldObj, myname, this.posX + (double)this.worldObj.rand.nextFloat(), this.posY + 0.01, this.posZ + (double)this.worldObj.rand.nextFloat());
					if (newent != null) {
				...
				return true;
			} else if (this.is_activated == 2 && par1EntityPlayer.getDistanceSqToEntity(this) < 16.0D) {
				if (this.isSitting()) {
	...
	private boolean isSuitableTarget(EntityLivingBase par1EntityLiving, boolean par2) {
		if (this.worldObj.difficultySetting == EnumDifficulty.PEACEFUL) {
			...
			if (this.isSitting()) {
				double dx = (double)this.px - par1EntityLiving.posX;
				double dy = (double)this.py - par1EntityLiving.posY;
				double dz = (double)this.pz - par1EntityLiving.posZ;
				if (dx * dx + dy * dy + dz * dz > (double)144.0F) {
					return false;
				}
			}
			...
			if (par1EntityLiving instanceof EntityMob) {
				return true;
			} else if (par1EntityLiving instanceof EntityCannonFodder) {
				...
				if (p.capabilities.isCreativeMode) {
					return false;
				} else if (this.name_one != null && this.name_one.equals(p.getUniqueID().toString())) {
					return false;
				} else {
	...
	public int getTotalArmorValue() {
		return this.is_activated == 2 ? 3 : 0;
	}
			...
			if (this.is_activated == 2) {
				...
				if (this.worldObj.difficultySetting != EnumDifficulty.PEACEFUL && this.worldObj.rand.nextInt(pfreq) == 1) {
					...
					if (e != null) {
						...
						if (this.getDistanceSqToEntity(e) < 9.0D && (this.rand.nextInt(sfreq + 1) == 0 || this.rand.nextInt(sfreq) == 1)) {
