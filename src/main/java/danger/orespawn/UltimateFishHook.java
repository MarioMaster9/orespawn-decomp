...
public class UltimateFishHook extends EntityFishHook
{
	...
	private static final List orespawn_lava_fish = Arrays.asList(
	...
	private static final List orespawn_fish = Arrays.asList(
	...
	public void onUpdate()
	{
		...
		else
		{
			if (!this.worldObj.isRemote)
			{
				...
				if (this.field_146042_b.isDead || !this.field_146042_b.isEntityAlive() || itemstack == null || itemstack.getItem() != OreSpawnMain.MyUltimateFishingRod || this.getDistanceSqToEntity(this.field_146042_b) > (double)1024.0F)
				{
					this.setDead();
					...
					else
					{
						...
						if (this.ticks_catchable > 0)
						{
							...
							if (this.ticks_catchable <= 0)
							{
								...
								worldserver.func_147487_a("bubble", this.posX, (double)(f1 + 1.0F), this.posZ, (int)(1.0F + this.width * 20.0F), (double)this.width, 0.0D, (double)this.width, (double)0.2F);
								worldserver.func_147487_a("wake", this.posX, (double)(f1 + 1.0F), this.posZ, (int)(1.0F + this.width * 20.0F), (double)this.width, 0.0D, (double)this.width, (double)0.2F);
								this.fish_on_hook = MathHelper.getRandomIntegerInRange(this.rand, 10, 30);
							}
							else
							{
								this.fish_direction = (float)((double)this.fish_direction + this.rand.nextGaussian() * 4.0D);
								f1 = this.fish_direction * ((float)Math.PI / 180F);
								f7 = MathHelper.sin(f1);
								f2 = MathHelper.cos(f1);
								d11 = this.posX + (double)(f7 * (float)this.ticks_catchable * 0.1F);
								d5 = (double)((float)MathHelper.floor_double(this.boundingBox.minY) + 1.0F);
								d6 = this.posZ + (double)(f2 * (float)this.ticks_catchable * 0.1F);
								...
								float f3 = f7 * 0.04F;
								float f4 = f2 * 0.04F;
								worldserver.func_147487_a("wake", d11, d5, d6, 0, (double)f4, 0.01, (double)(-f3), 1.0D);
								worldserver.func_147487_a("wake", d11, d5, d6, 0, (double)(-f4), 0.01, (double)f3, 1.0D);
							}
						}
						else if (this.fish_wait_time > 0)
						{
							...
							{
								f7 = MathHelper.randomFloatClamp(this.rand, 0.0F, 360.0F) * ((float)Math.PI / 180F);
	...
	public int func_146034_e()
	{
		...
		else
		{
			...
			{
				EntityItem entityitem = new EntityItem(this.worldObj, this.posX, this.posY + 1.25D, this.posZ, this.func_146033_f());
