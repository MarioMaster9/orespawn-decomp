...
public class Trees
{
	...
	private void WindTreeBranch(World world, int x, int y, int z, int length, int dirx, int dirz)
	{
		...
		for (i = 1; i <= length; i++) {
			...
			if (i < length / 3) {
				if (Blocks.air == world.getBlock(x + i * dirx, y + 2, z + i * dirz)) {
	...
	public void WindTree(World world, int x, int y, int z, int dir)
	{
		...
		height = world.rand.nextInt(8) + 40;
		width = world.rand.nextInt(4) + 8;
	...
	public void SkyTree(World world, int x, int y, int z)
	{
		...
		height = world.rand.nextInt(15) + 190;
		
		if (height - y < 20) return;
		
		width = world.rand.nextInt(10) + 25;
	...
	public void DuplicatorTree(World world, int x, int y, int z)
	{
		...
		for (tries = 0; tries < 20 && (bidm == Blocks.air || bidm == OreSpawnMain.MyDT); tries++) {
			...
			meta = world.getBlockMetadata(x + i, realy + 1, z + j);
	...
	private void make_leaves(World world, int x, int y, int z)
	{
		...
		for (l1 = -3; l1 <= 3; ++l1) {
			for (l2 = -3; l2 <= 3; ++l2) {
				for (l3 = 0; l3 <= 2; ++l3) {
					bid = world.getBlock(x + l1, y + l3, z + l2);
					if (bid == Blocks.air) OreSpawnMain.setBlockFast(world, x + l1, y + l3, z + l2, OreSpawnMain.MyExperienceLeaves, 0, 2);
				}
			}
		}
	}
	...
	{
		int unused1, unused2, i, j, k, tries, realy = y;
	...
	public void FairyTree(World world, int x, int y, int z)
	{
		...
		if (chest != null)
		{
			...
			WeightedRandomChestContent.generateChestContents(world.rand, this.CrystalChestContentsList, chest, 1 + world.rand.nextInt(5));
		}
	...
	private void make_crystal_leaves(World world, int x, int y, int z)
	{
		...
		for (l1 = -2; l1 <= 2; ++l1) {
			for (l2 = -2; l2 <= 2; ++l2) {
				for (l3 = 0; l3 <= 1; ++l3) {
					bid = world.getBlock(x + l1, y + l3, z + l2);
					if (bid == Blocks.air) OreSpawnMain.setBlockFast(world, x + l1, y + l3, z + l2, OreSpawnMain.MyCrystalLeaves3, 0, 2);
				}
			}
		}
	}
	...
	{
		int i, j, k, grow, iter, width, spread, randy;
		...
		j = 3 + world.rand.nextInt(3);
		spread = 0;

		for (iter = 0; iter < nc; iter++) {
			grow = 4 + world.rand.nextInt(3);
			width = 1 + world.rand.nextInt(3);
			...
			for (i = -width; i <= width; i++) {
				for (k = -width; k <= width; k++) {
					...
					if (i == -width || i == width || k == -width || k == width) this.make_crystal_castle_leaves(world, x + i + spread, y + j + randy, z + k);
					if (iter != 0 && i == 0 && k == 0) this.addSomething(world, x + i + spread, y + j + randy, z + k);
					if (i == -width && (k == -width || k == width)) OreSpawnMain.setBlockFast(world, x + i + spread, y + j + randy + 1, z + k, OreSpawnMain.CrystalTorch, 0, 2);
					if (i == width && (k == -width || k == width)) OreSpawnMain.setBlockFast(world, x + i + spread, y + j + randy + 1, z + k, OreSpawnMain.CrystalTorch, 0, 2);
				}
			}
			if (iter != 0) {
				...
				for (i = -width; i <= width; i++) {
					for (k = -width; k <= width; k++) {
						OreSpawnMain.setBlockFast(world, x + i - spread, y + j + randy, z + k, OreSpawnMain.MyCrystalTreeLog, 0, 2);
						if (i == -width || i == width || k == -width || k == width) this.make_crystal_castle_leaves(world, x + i - spread, y + j + randy, z + k);
						if (i == 0 && k == 0) this.addSomething(world, x + i - spread, y + j + randy, z + k);
						if (i == -width && (k == -width || k == width)) OreSpawnMain.setBlockFast(world, x + i - spread, y + j + randy + 1, z + k, OreSpawnMain.CrystalTorch, 0, 2);
						if (i == width && (k == -width || k == width)) OreSpawnMain.setBlockFast(world, x + i - spread, y + j + randy + 1, z + k, OreSpawnMain.CrystalTorch, 0, 2);
					}
				}
				...
				for (i = -width; i <= width; i++) {
					for (k = -width; k <= width; k++) {
						...
						if (i == -width || i == width || k == -width || k == width) this.make_crystal_castle_leaves(world, x + i, y + j + randy, z + k + spread);
						if (i == 0 && k == 0) this.addSomething(world, x + i, y + j + randy, z + k + spread);
						if (i == -width && (k == -width || k == width)) OreSpawnMain.setBlockFast(world, x + i, y + j + randy + 1, z + k + spread, OreSpawnMain.CrystalTorch, 0, 2);
						if (i == width && (k == -width || k == width)) OreSpawnMain.setBlockFast(world, x + i, y + j + randy + 1, z + k + spread, OreSpawnMain.CrystalTorch, 0, 2);
					}
				}
				width = 1 + world.rand.nextInt(3 + iter);
			...
			if (iter >= 2) {
				...
				for (i = -width; i <= width; i++) {
					for (k = -width; k <= width; k++) {
						...
						if (i == -width || i == width || k == -width || k == width) this.make_crystal_castle_leaves(world, x + i + spread, y + j + randy, z + k + spread);
						if (i == 0 && k == 0) this.addSomething(world, x + i + spread, y + j + randy, z + k + spread);
						if (i == -width && (k == -width || k == width)) OreSpawnMain.setBlockFast(world, x + i + spread, y + j + randy + 1, z + k + spread, OreSpawnMain.CrystalTorch, 0, 2);
						if (i == width && (k == -width || k == width)) OreSpawnMain.setBlockFast(world, x + i + spread, y + j + randy + 1, z + k + spread, OreSpawnMain.CrystalTorch, 0, 2);
					}
				}
				width = 1 + world.rand.nextInt(3 + iter);
