...
public class ModelFairy extends ModelBase
{
	...
	public ModelFairy(float f1)
	{
		...
		this.setRotation(this.lleg1, (-(float)Math.PI / 4F), 0.0F, 0.0F);
	...
	public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5)
	{
		...
		this.lwing1.rotateAngleY = -0.6F + MathHelper.cos(f2 * this.wingspeed) * (float)Math.PI * 0.35F;
		this.rwing1.rotateAngleY = -2.55F - MathHelper.cos(f2 * this.wingspeed) * (float)Math.PI * 0.35F;
		this.lwing2.rotateAngleY = -0.6F + MathHelper.cos(f2 * this.wingspeed * 0.85F) * (float)Math.PI * 0.25F;
		this.rwing2.rotateAngleY = -2.55F - MathHelper.cos(f2 * this.wingspeed * 0.85F) * (float)Math.PI * 0.25F;
		
		this.head.rotateAngleY = (float)Math.toRadians((double)f3) * 0.45F;
		if (this.head.rotateAngleY > 0.45F) this.head.rotateAngleY = 0.45F;
		if (this.head.rotateAngleY < -0.45F) this.head.rotateAngleY = -0.45F;
		this.head.rotateAngleX = (float)Math.toRadians((double)f4);
		
		this.larm.rotateAngleX = -0.2F + MathHelper.cos(f2 * this.wingspeed * 0.15F) * (float)Math.PI * 0.05F;
		this.rarm.rotateAngleX = -0.2F + MathHelper.cos(f2 * this.wingspeed * 0.12F) * (float)Math.PI * 0.05F;
		this.larm.rotateAngleZ = -0.15F + MathHelper.cos(f2 * this.wingspeed * 0.1F) * (float)Math.PI * 0.03F;
		this.rarm.rotateAngleZ = 0.15F + MathHelper.cos(f2 * this.wingspeed * 0.11F) * (float)Math.PI * 0.03F;
