...
public class TheKing extends EntityMob
{
		...
		if (!this.isDead) {
			...
			if (this.isEnd == 1) {
				...
				else if (this.worldObj.rand.nextInt(attrand) == 0)
				{
					...
					{
						EntityLiving entityLiving = (EntityLiving)spawnCreature(this.worldObj, "KingHead", this.posX, this.posY + 20.0D, this.posZ);
					}
	...
	public boolean MyCanSee(EntityLivingBase e) {
		...
		for (int i = 0; i < nblks; i++) {
			...
			if (bid != Blocks.flowing_water && bid != Blocks.water && bid != Blocks.leaves && bid != Blocks.vine && bid != Blocks.air) {
				return false;
			}
		}
