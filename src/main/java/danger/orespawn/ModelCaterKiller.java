...
public class ModelCaterKiller extends ModelBase
{
	...
	public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5)
	{
		...
		if (e.getAttacking() != 0) {
			newangle = MathHelper.cos(f2 * 1.7F * this.wingspeed) * (float)Math.PI * 0.07F;
		} else {
			newangle = MathHelper.cos(f2 * 1.3F * this.wingspeed) * (float)Math.PI * 0.025F;
		}
		...
		newangle = MathHelper.cos(f2 * 2.11F * this.wingspeed) * (float)Math.PI * 0.08F;
		this.ltusk2.rotateAngleY = 0.802F + newangle;
		newangle = MathHelper.cos(f2 * 2.3F * this.wingspeed) * (float)Math.PI * 0.08F;
		...
		for (i = 0; i < 3; i++) {
			...
			newangle = MathHelper.cos((float)((double)(f2 * 0.91F * this.wingspeed) + (Math.PI / 8D) * (double)i)) * (float)Math.PI * 0.08F;
			this.seg1lspike.rotateAngleZ = newangle;
			this.seg1rspike.rotateAngleZ = -newangle;
			
			if (e.getAttacking() != 0) {
				newangle = MathHelper.cos((float)((double)(f2 * 2.91F * this.wingspeed) + (Math.PI / 8D) * (double)i)) * (float)Math.PI * 0.15F;
			} else {
				newangle = MathHelper.cos((float)((double)(f2 * 0.35F * this.wingspeed) + (Math.PI / 8D) * (double)i)) * (float)Math.PI * 0.04F;
			}
		...
		for (i = 0; i < 6; i++)
		{
			...
			newangle = MathHelper.cos((float)((double)(f2 * 0.4F * this.wingspeed) - (Math.PI / 8D) * (double)i)) * (float)Math.PI * 0.07F;
		...
		newangle = MathHelper.cos(f2 * 0.81F * this.wingspeed) * (float)Math.PI * 0.04F;
		this.seg3lbackspike.rotateAngleX = -0.977F + newangle;
		newangle = MathHelper.cos(f2 * 0.87F * this.wingspeed) * (float)Math.PI * 0.04F;
		this.seg3rbackspike.rotateAngleX = -0.977F + newangle;
		
		newangle = MathHelper.cos(f2 * 1.11F * this.wingspeed) * (float)Math.PI * 0.04F;
		this.seg3lbackspike.rotateAngleY = 0.28F + newangle;
		newangle = MathHelper.cos(f2 * 1.3F * this.wingspeed) * (float)Math.PI * 0.04F;
