...
public class ModelGhostSkelly extends ModelBase
{
	...
	public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5)
	{
		...
		this.larm.rotateAngleZ = this.lsleeve.rotateAngleZ = this.lchains.rotateAngleZ = MathHelper.cos(f2 * 0.2F) * (float)Math.PI * 0.05F;
		this.rarm.rotateAngleZ = this.rsleeve.rotateAngleZ = this.rchains.rotateAngleZ = MathHelper.cos(f2 * 0.22F) * (float)Math.PI * 0.05F;
		this.larm.rotateAngleY = this.lsleeve.rotateAngleY = this.lchains.rotateAngleY = MathHelper.cos(f2 * 0.24F) * (float)Math.PI * 0.05F;
		this.rarm.rotateAngleY = this.rsleeve.rotateAngleY = this.rchains.rotateAngleY = MathHelper.cos(f2 * 0.26F) * (float)Math.PI * 0.05F;
		
		
		
		newangle = MathHelper.cos(f2 * 0.05F) * (float)Math.PI * 2.0F;
		newrf1 = f2 * 0.05F % ((float)Math.PI * 2F);
		newrf1 = Math.abs(newrf1);
		
		if (newrf1 < r.rf2)
		{
			r.ri2 = 0;
			if (e.worldObj.rand.nextInt(3) == 1) r.ri2 |= 1;
		}
		r.rf2 = newrf1;
		if ((r.ri2 & 1) == 0) {
			newangle = 0.0F;
