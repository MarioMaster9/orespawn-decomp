...
public class CrystalAntBlock extends Block {
	...
	public IIcon getBlockTexture(IBlockAccess par1IBlockAccess, int par2, int par3, int par4, int par5) {
		if (this.field_94364_a == null) {
			...
			return par5 == 0 ? this.field_94364_a[1] : this.field_94364_a[2];
		}
	}
				...
				for (int i = 0; i < howmany; i++) {
					if (this == OreSpawnMain.MyAntBlock) {
						if (OreSpawnMain.BlackAntEnable != 0) {
							spawnCreature(par1World, "Ant", (double)par2 + 0.5D, (double)par3 + 1.01, (double)par4 + 0.5D);
						}
					} else if (this == OreSpawnMain.MyRedAntBlock) {
						if (OreSpawnMain.RedAntEnable != 0) {
							spawnCreature(par1World, "Red Ant", (double)par2 + 0.5D, (double)par3 + 1.01, (double)par4 + 0.5D);
						}
					} else if (this == OreSpawnMain.MyUnstableAntBlock) {
						if (OreSpawnMain.UnstableAntEnable != 0) {
							spawnCreature(par1World, "Unstable Ant", (double)par2 + 0.5D, (double)par3 + 1.01, (double)par4 + 0.5D);
						}
					} else if (this == OreSpawnMain.TermiteBlock) {
						if (OreSpawnMain.TermiteEnable != 0) {
							spawnCreature(par1World, "Termite", (double)par2 + 0.5D, (double)par3 + 1.01, (double)par4 + 0.5D);
						}
						...
						spawnCreature(par1World, "Rainbow Ant", (double)par2 + 0.5D, (double)par3 + 1.01, (double)par4 + 0.5D);
					}
				}
			}
		}
