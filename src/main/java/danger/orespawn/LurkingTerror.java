...
public class LurkingTerror extends EntityMob {
	...
	public boolean attackEntityAsMob(Entity par1Entity) {
		boolean var4 = par1Entity.attackEntityFrom(DamageSource.causeMobDamage(this), 5.0F);
		return var4;
	}
	...
	public boolean getCanSpawnHere() {
		...
		if (!this.isValidLightLevel()) {
			...
			return false;
		} else if (this.worldObj.rand.nextInt(2) != 1) {
			return false;
		} else if (this.worldObj.provider.dimensionId == OreSpawnMain.DimensionID6 && this.worldObj.rand.nextInt(6) != 0) {
			return false;
		} else {
			target = (LurkingTerror)this.worldObj.findNearestEntityWithinAABB(LurkingTerror.class, this.boundingBox.expand(32.0D, 16.0D, 32.0D), this);
			if (target != null) {
				return false;
			} else if (this.posY < 10.0D) {
				return false;
			} else {
				return true;
			}
	...
	protected Item getDropItem() {
		...
		if (i == 0) {
			...
			return i == 1 ? Items.flint : Items.feather;
		}
	}
}
