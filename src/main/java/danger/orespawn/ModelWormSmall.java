...
public class ModelWormSmall extends ModelBase
{
	...
	public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5)
	{
		...
		float d1 = (float)(Math.sin((double)newangle) * 5.0D);
		float d2 = (float)(Math.cos((double)newangle) * 5.0D);
		...
		this.body.rotationPointY = (float)((double)this.tail.rotationPointY - 5.0D + (5.0D - (double)d3));
		
		newangle = MathHelper.cos(f2 * 0.45F) * (float)Math.PI * 0.15F;
		this.body.rotateAngleX = newangle;
		d1 = (float)(Math.sin((double)newangle) * 5.0D);
		d2 = (float)(Math.cos((double)newangle) * 5.0D);
		...
		this.head.rotationPointY = (float)((double)this.body.rotationPointY - 5.0D + (5.0D - (double)d3));
		
		this.head.rotateAngleX = 0.62F + MathHelper.cos(f2 * 0.65F) * (float)Math.PI * 0.15F;
		this.head.rotateAngleZ = MathHelper.cos(f2 * 0.3F) * (float)Math.PI * 0.05F;
