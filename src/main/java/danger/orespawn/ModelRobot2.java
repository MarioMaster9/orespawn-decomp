...
public class ModelRobot2 extends ModelBase
{
	...
	public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5)
	{
		...
		float newangle;
		if ((double)f1 > 0.1) {
			newangle = MathHelper.cos(f2 * 0.3F * this.wingspeed) * (float)Math.PI * 0.12F * f1;
		} else {
			newangle = 0.0F;
		}
		...
		newangle = MathHelper.sin((float)Math.toRadians((double)(f2 * 20.0F * this.wingspeed)));
		float nextangle = MathHelper.sin((float)Math.toRadians((double)(f2 * 20.0F * this.wingspeed + 1.5F)));
		...
		if (nextangle > 0.0F && newangle < 0.0F)
		{
			...
			if (e.getAttacking() == 0) {
				...
				while (r.ri1 == 0) r.ri1 = e.worldObj.rand.nextInt(4);
			}
		}

		newangle = (float)Math.toRadians((double)(f2 * 20.0F * this.wingspeed));
