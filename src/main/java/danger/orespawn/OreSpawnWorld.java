...
public class OreSpawnWorld implements IWorldGenerator
{
	...
	public void generate(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator, IChunkProvider chunkProvider)
	{
		...
		if (world.provider.dimensionId == OreSpawnMain.DimensionID4)
		{
			...
			if (recently_placed == 0 && random.nextInt(100) == 0) {
				if (this.D4BigSpaceCheck(world, chunkX * 16, 7, chunkZ * 16)) {
				...
				for (int j = 0; j < nc; j++) {
					...
					for (int posY = 100; posY > 40 && world.isAirBlock(posX, posY, posZ); --posY) {
						if (world.getBlock(posX, posY - 1, posZ) == Blocks.grass) {
							...
							for (int var14 = 1; var14 < corn_height; ++var14) {
								OreSpawnMain.setBlockFast(world, posX, posY + var14, posZ, OreSpawnMain.MyCornPlant4, 0, 2);
							}
				...
				for (int j = 0; j < 5; j++) {
					...
					for (int posY = 100; posY > 40 && world.isAirBlock(posX, posY, posZ); --posY) {
						if (world.getBlock(posX, posY - 1, posZ) == Blocks.grass) {
							...
							for (int var13 = 1; var13 < corn_height; ++var13) {
								OreSpawnMain.setBlockFast(world, posX, posY + var13, posZ, OreSpawnMain.MyTomatoPlant4, 0, 2);
							}
	...
	public boolean addSpiderHangout(World world, Random random, int chunkX, int chunkZ) {
		if (random.nextInt(350) != 0) {
			return false;
		} else if (OreSpawnMain.SpiderDriverEnable == 0) {
			return false;
		} else {
			for (int i = 0; i < 4; i++) {
				int posX = chunkX + random.nextInt(16);
				...
				for (int i = 0; i < 4; i++) {
					...
					for (int posY = 100; posY > 40 && world.isAirBlock(posX, posY, posZ); --posY) {
						if (world.getBlock(posX, posY - 1, posZ) == Blocks.grass) {
							...
							} else if (OreSpawnMain.BlackAntEnable != 0) {
	...
	public void addCrystalChest(World world, int x, int y, int z, int dir) {
		...
		if (i == 0) {
			...
			if (chest != null) {
				Trees var10001 = OreSpawnMain.OreSpawnTrees;
				WeightedRandomChestContent.generateChestContents(world.rand, Trees.CrystalChestContentsList, chest, 1 + world.rand.nextInt(3));
			}
		} else {
	...
	public boolean addHugeTree(World world, Random random, int chunkX, int chunkZ, Chunk chunk) {
		...
		if (random.nextInt(50) != 0) {
			return false;
		} else if (OreSpawnMain.LessLag == 1 && random.nextInt(2) != 0) {
			return false;
		} else if (OreSpawnMain.LessLag == 2 && random.nextInt(4) != 0) {
			return false;
		} else {
				...
				for (int i = 0; i < 8; i++) {
					...
					for (int posY = 100; posY > 40 && world.isAirBlock(posX, posY, posZ); --posY) {
						if (world.getBlock(posX, posY - 1, posZ) == Blocks.grass) {
							...
							} else if (random.nextInt(50) == 1 && OreSpawnMain.enableduplicatortree != 0) {
			...
			for (int posY = 128; posY > 40; --posY) {
				if (world.isAirBlock(posX, posY, posZ) && world.getBlock(posX, posY - 1, posZ) == OreSpawnMain.CrystalGrass) {
					...
					for (int var11 = -2; var11 <= 2; ++var11) {
						for (int j = -2; j <= 2; j++) {
							Block bid = world.getBlock(posX + var11, posY - 1, posZ + j);
				...
				if (bid == Blocks.grass) {
					for (int x = -5; x < 60; ++x) {
						for (int z = -5; z < 70; ++z) {
							...
							if (bid != Blocks.air && bid != Blocks.log && bid != OreSpawnMain.MyAppleLeaves && bid != OreSpawnMain.MyScaryLeaves && bid != Blocks.air) {
								return false;
							}
						}
					}
	...
	private boolean D4BigSpaceCheck(World world, int posX, int posY, int posZ) {
		for (int i = -25; i < 40; i++) {
			for (int k = -25; k < 30; k++) {
				...
				if (bid != Blocks.air && bid != Blocks.log && bid != OreSpawnMain.MyAppleLeaves && bid != OreSpawnMain.MyScaryLeaves) {
					return false;
				}
			}
		}
