		...
		if (cposx - pposx == 0 || cposz - pposz == 0)
		{
			...
			for (int i = 0; i < height; i++)
			{
				...
				for (int k = 0; k < length; k++)
				{
					...
					for (int j = -width; j <= width; j++)
					{
						...
						if (i == 0) world.setBlock(x + k * deltax + j * deltaz, y + i - 1, z + k * deltaz + j * deltax, Blocks.grass, 0, 2);
			...
			for (int k = 1; k < length - 1; k++)
			{
				...
				for (int j = -width; j <= width; j++)
				{
					...
					if (i == 11) {
						world.setBlock(x + k * deltax + j * deltaz, y - 2, z + k * deltaz + j * deltax, Blocks.cobblestone, 0, 2);
						world.setBlock(x + k * deltax + j * deltaz, y - 1, z + k * deltaz + j * deltax, Blocks.sand, 0, 2);
