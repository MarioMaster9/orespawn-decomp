...
public class ChunkProviderOreSpawn5 implements IChunkProvider
{
	...
	public void addPinkTourmaline(World world, Random random, int chunkX, int chunkZ, Chunk chunk)
	{
		...
		patchy = 1 + random.nextInt(10);

		for(int i = 0; i < patchy; i++) {
			dx = (random.nextFloat()-random.nextFloat());
			dz = (random.nextFloat()-random.nextFloat());
			dy = 0.5F + (random.nextFloat()/2.0F);
			width = random.nextInt(2); //stay thin!
			length = 1 + width*3 + random.nextInt(15);
			rx = randPosX;
			ry = randPosY;
			rz = randPosZ;
	...
	public void addTigersEye(World world, Random random, int chunkX, int chunkZ, Chunk chunk)
	{
		...
		for(int i = 0; i < patchy; i++) {
			...
			dy = 0.5F + (random.nextFloat()/2.0F);
			width = 0;
			length = width*3 + random.nextInt(6);
			rx = (float)randPosX;
			ry = (float)randPosY;
			rz = (float)randPosZ;
			for(iy=0;iy<=length;iy++){
	...
	public void addCrystalTrees(World world, Random random, int chunkX, int chunkZ, Chunk chunk)
	{
		...
		for(int i = 0; i < howmany; i++) {
			...
			for(int posY = 128; posY > 40; posY--) //Start high and search down until we find a grass/dirt block.
			{
				...
				{
					if(what == 0){
						this.TallCrystalTree(world, posX, posY, posZ, chunk); break;
					}else{
	...
	public void TallCrystalTree(World world, int x, int y, int z, Chunk chunk)
	{
		...
		j = i + world.rand.nextInt(18 - OreSpawnMain.LessLag * 2);
	...
	public boolean generateOre(World par1World, Random par2Random, int par3, int par4, int par5, Chunk chunk, Block newbid, int numberOfBlocks, Block oldbid)
	{
		...
		double d0 = (double)((float)(par3 + 8) + MathHelper.sin(f) * (float)numberOfBlocks / 8.0F);
		double d1 = (double)((float)(par3 + 8) - MathHelper.sin(f) * (float)numberOfBlocks / 8.0F);
		double d2 = (double)((float)(par5 + 8) + MathHelper.cos(f) * (float)numberOfBlocks / 8.0F);
		double d3 = (double)((float)(par5 + 8) - MathHelper.cos(f) * (float)numberOfBlocks / 8.0F);
	...
	public void addCrystalFlowers(World world, Random random, int chunkX, int chunkZ, Chunk chunk)
	{
		...
		for (int i = 0; i < howmany; i++) {
			...
			for (int posY = 128; posY > 40; --posY)
			{
				...
				{
					if (what == 0)
						{OreSpawnMain.setBlockIDWithMetadataInChunk(chunk, posX, posY, posZ, OreSpawnMain.CrystalFlowerRedBlock, 0); break; }
					if (what == 1)
						{OreSpawnMain.setBlockIDWithMetadataInChunk(chunk, posX, posY, posZ, OreSpawnMain.CrystalFlowerGreenBlock, 0); break;}
					if (what == 2)
						{OreSpawnMain.setBlockIDWithMetadataInChunk(chunk, posX, posY, posZ, OreSpawnMain.CrystalFlowerBlueBlock, 0); break;}
					if (what == 3)
						{OreSpawnMain.setBlockIDWithMetadataInChunk(chunk, posX, posY, posZ, OreSpawnMain.CrystalFlowerYellowBlock, 0); break;}
