...
public class Vortex extends EntityMob {
	...
	public void onUpdate() {
		...
		if (e != null) {
			...
			if (this.worldObj.isRemote) {
				for (int i = 0; i < 20; i++) {
					double d = this.worldObj.rand.nextDouble() * (double)3.5F;
					d *= d;
	...
	protected void updateAITasks() {
		...
		if (!this.isDead) {
			...
			if (e != null) {
				...
				if (d < (double)81.0F && this.winded == 0) {
					...
					e.addVelocity(Math.cos(a) * (10.0D - Math.sqrt(d)) * 0.1D, (10.0D - Math.sqrt(d)) * (double)0.05F * pm, Math.sin(a) * (10.0D - Math.sqrt(d)) * 0.1D);
				}
	...
	public boolean getCanSpawnHere() {
		...
		for (int var11 = -2; var11 <= 2; ++var11) {
			for (int j = -2; j <= 2; j++) {
				for (int i = 1; i < 4; i++) {
					Block bid = this.worldObj.getBlock((int)this.posX + j, (int)this.posY + i, (int)this.posZ + var11);
					if (bid != Blocks.air) {
						return false;
					}
				}
