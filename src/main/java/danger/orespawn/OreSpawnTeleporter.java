...
public class OreSpawnTeleporter extends Teleporter
{
	...
	private boolean isGroundBlock(Block bid)
	{
		...
		if (bid == Blocks.dirt) return true;
		if (bid == Blocks.grass) return true;
		if (bid == Blocks.stone) return true;
		if (bid == Blocks.end_stone) return true;
		if (bid == Blocks.netherrack) return true;
		if (bid == Blocks.cobblestone) return true;
		if (bid == Blocks.sand) return true;
		if (bid == Blocks.sandstone) return true;
		if (bid == Blocks.farmland) return true;
		return false;
	}
	...
	public boolean justPutMe(Entity par1Entity)
	{
		...
		double newX = (double)posX;
		double newZ = (double)posZ;
	...
	public void sendToThisDimension(Entity e, double newX, double newY, double newZ, int ro)
	{
		...
		Entity var6 = EntityList.createEntityByName(EntityList.getEntityString(e), this.world);
		if (var6 != null)
		{
