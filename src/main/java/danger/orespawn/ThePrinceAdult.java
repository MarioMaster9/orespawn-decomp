	...
	public void fly_with_rider()
	{
		...
		if (this.worldObj.rand.nextInt(5) == 1 && this.worldObj.difficultySetting != EnumDifficulty.PEACEFUL)
		{
			...
			if (e != null)
			{
				...
				} else if (this.getDistanceSqToEntity(e) < 625.0D && !this.isInWater()) {
					if (this.getThePrinceAdultFire() != 0) {
			...
			if (e != null)
			{
				...
				} else
				{
					...
					if (this.getDistanceSqToEntity(e) < (double)((10.0F + e.width / 2.0F) * (10.0F + e.width / 2.0F))) {
						...
						do_new = 1;
					} else if (this.getDistanceSqToEntity(e) < 600.0D && !this.isInWater())
					{
						if (this.getThePrinceAdultFire() != 0 && this.worldObj.rand.nextInt(2) == 1) {
							this.shoot_something(e.posX, e.posY, e.posZ);
						}
					}
