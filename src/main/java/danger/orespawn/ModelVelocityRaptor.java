...
public class ModelVelocityRaptor extends ModelBase
{
	...
	public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5)
	{
		...
		newangle = MathHelper.cos(f2 * 1.25F * this.wingspeed * hf) * (float)Math.PI * 0.1F * hf;
		...
		newangle = MathHelper.cos(f2 * 0.3F) * (float)Math.PI * 0.05F;
		...
		if (c.isSitting()) {
			...
			newangle = MathHelper.cos(f2 * 1.4F * this.wingspeed * hf) * (float)Math.PI * 0.25F * hf;
		}
