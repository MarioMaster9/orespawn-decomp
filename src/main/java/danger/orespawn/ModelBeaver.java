...
public class ModelBeaver extends ModelBase
{
	...
	public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5)
	{
		...
		newangle = MathHelper.cos(f2 * 3.7F * this.wingspeed) * (float)Math.PI * 0.45F * f1;
		this.rff.rotateAngleX = this.lrf.rotateAngleX = newangle;
		this.lff.rotateAngleX = this.rrf.rotateAngleX = -newangle;
		
		newangle = MathHelper.cos(f2 * 2.7F * this.wingspeed) * (float)Math.PI * 0.25F;
