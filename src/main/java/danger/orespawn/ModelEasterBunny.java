...
public class ModelEasterBunny extends ModelBase
{
	...
	public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5)
	{
		...
		if ((double)f1 > 0.1) {
			newangle = MathHelper.cos(f2 * 2.6F * this.wingspeed) * (float)Math.PI * 0.15F * f1;
			newangle2 = MathHelper.cos(f2 * 1.3F * this.wingspeed) * (float)Math.PI * 0.1F * f1;
		} else {
			newangle = 0.0F;
			newangle2 = MathHelper.cos(f2 * 1.3F * this.wingspeed) * (float)Math.PI * 0.01F;
		}
