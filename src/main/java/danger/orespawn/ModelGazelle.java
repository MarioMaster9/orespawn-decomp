...
public class ModelGazelle extends ModelBase
{
	...
	public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5)
	{
		...
		if ((double)f1 > 0.1) {
			newangle = MathHelper.cos(f2 * 1.1F * this.wingspeed) * (float)Math.PI * 0.12F * f1;
		} else {
			newangle = 0.0F;
		}
		...
		newangle = MathHelper.cos(f2 * 0.5F) * (float)Math.PI * 0.02F;
		...
		if (!g.isSitting()) {
			this.tail.rotateAngleX = 1.0F + MathHelper.cos(f2 * 0.1F) * (float)Math.PI * 0.06F;
		}
