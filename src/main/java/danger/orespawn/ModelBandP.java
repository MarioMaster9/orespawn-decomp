...
public class ModelBandP extends ModelBase
{
	...
	public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5)
	{
		...
		if ((double)f1 > 0.1) {
			...
			newangle2 = MathHelper.cos(f2 * 2.6F * this.wingspeed) * (float)Math.PI * 0.025F * f1;
			newangle3 = newangle;
		} else {
			newangle = 0.0F;
			newangle2 = MathHelper.cos(f2 * 0.6F * this.wingspeed) * (float)Math.PI * 0.005F;
			newangle3 = MathHelper.cos(f2 * 0.3F * this.wingspeed) * (float)Math.PI * 0.02F;
		}
