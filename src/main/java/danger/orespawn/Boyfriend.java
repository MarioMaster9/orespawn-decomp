...
public class Boyfriend extends EntityTameable implements IRangedAttackMob
{
	...
	public Boyfriend(World par1World)
	{
		...
		this.tasks.addTask(2, new EntityAITempt(this, 1.25D, Items.cooked_beef, false));
		...
		if (OreSpawnMain.PlayNicely == 0) this.targetTasks.addTask(4, new MyEntityAIJealousy(this, Boyfriend.class, 6.0F, 5, true));
		if (OreSpawnMain.PlayNicely == 0) this.targetTasks.addTask(5, new MyEntityAIJealousy(this, Boyfriend.class, 3.0F, 15, true));
		...
		if (stack != null && !this.isSitting())
		{
					...
					if (this.getHeldItem() != null)
					{
						if (this.getDistanceToEntity(victim) < 4.0F || stack.getItem() == OreSpawnMain.MyBertha && this.getDistanceToEntity(victim) < 10.0F)
						{
							...
							if (this.attackTime <= 0)
							{
								...
								{
									if (!this.worldObj.isRemote && this.voice_enable != 0) this.worldObj.playSoundAtEntity(this, "orespawn:b_fight", 0.5F, this.getSoundPitch());
						...
						else if (this.getDistanceToEntity(victim) < 7.0F && stack.getItem() != OreSpawnMain.MyUltimateBow)
						{
							...
							{
								if (!this.worldObj.isRemote && this.voice_enable != 0) this.worldObj.playSoundAtEntity(this, "orespawn:b_taunt", 0.5F, this.getSoundPitch());
			...
			} else
			{
				...
				if (this.had_target != 0) {
					this.had_target = 0;
					if (!this.worldObj.isRemote && this.voice_enable != 0) this.worldObj.playSoundAtEntity(this, "orespawn:b_woohoo", 0.4F, this.getSoundPitch());
				}
			}
		}
	}
	...
	public ResourceLocation getTexture()
	{
		if (this.wet_count <= 0)
		{
			...
			if (this.is_prince == 1) return PrinceTexture1;
			if (this.is_prince == 2) return PrinceTexture2;
	...
	public boolean interact(EntityPlayer par1EntityPlayer)
	{
		...
		if (var2 != null && (var2.getItem() == Items.cooked_beef || var2.getItem() == OreSpawnMain.MyPeacock) && par1EntityPlayer.getDistanceSqToEntity(this) < 16.0D)
		{
			if (!this.isTamed())
			{
				if (!this.worldObj.isRemote)
		...
		else if (this.isTamed() && var2 != null && var2.getItem() == OreSpawnMain.MyAmethyst && par1EntityPlayer.getDistanceSqToEntity(this) < 16.0D && this.func_152114_e(par1EntityPlayer))
		{
		...
		else if (this.isTamed() && var2 != null && (var2.getItem() == Items.leather || var2.getItem() == OreSpawnMain.MyPeacockFeather) && par1EntityPlayer.getDistanceSqToEntity(this) < 16.0D && this.func_152114_e(par1EntityPlayer))
		{
			...
			if (!this.worldObj.isRemote)
			{
				if (this.wet_count > 0 || this.isInWater() || this.handleLavaMovement()) {
					this.which_wet_guy++;
					if (this.which_wet_guy > 17) this.which_wet_guy = 0;
					this.setWetTameSkin(this.which_wet_guy);
					this.worldObj.setEntityState(this, (byte)7);
					if (this.isInWater() || this.handleLavaMovement()) {
						this.wet_count = 500;
					...
					this.which_guy++;
					if (this.which_guy > 27) this.which_guy = 0;
					this.setTameSkin(this.which_guy);
					this.worldObj.setEntityState(this, (byte)7);
				}
			}
		...
		else if (this.isTamed() && var2 == null && par1EntityPlayer.getDistanceSqToEntity(this) < 16.0D && this.func_152114_e(par1EntityPlayer))
		{
			...
			else if (!this.worldObj.isRemote)
			{
				...
				healthMessage = healthMessage.format("I have %d health. Thanks for asking!", Integer.valueOf(this.getBoyfriendHealth())); // calling a static method as if it's non static
	...
	public boolean isWheat(ItemStack par1ItemStack)
	{
		if (par1ItemStack != null) {
			if (par1ItemStack.getItem() == Items.cooked_beef || par1ItemStack.getItem() == OreSpawnMain.MyPeacock) return true;
		}
	...
	protected String getLivingSound()
	{
		...
		if (OreSpawnMain.bro_mode != 0 && this.worldObj.rand.nextInt(2) == 1) return null;
		...
		if (this.worldObj.rand.nextInt(11) == 1)
		{
			...
			if (this.worldObj.rand.nextInt(4) != 0)
			{
				...
				if (!this.worldObj.isDaytime()) {
					if (this.worldObj.canBlockSeeTheSky((int)this.posX, (int)this.posY, (int)this.posZ)) {
						if (this.worldObj.rand.nextInt(3) == 0) return "orespawn:b_dark";
						return null;
					}
				}
			}
			...
			if (this.isTamed())
			{
				...
				if (OreSpawnMain.bro_mode != 0) return "orespawn:bb_happy";
	...
	{
		if (OreSpawnMain.bro_mode != 0) return null;
