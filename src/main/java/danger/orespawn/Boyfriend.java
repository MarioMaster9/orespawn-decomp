...
public class Boyfriend extends EntityTameable implements IRangedAttackMob {
	...
	public boolean interact(EntityPlayer par1EntityPlayer) {
		...
		if (var2 != null && (var2.getItem() == Items.cooked_beef || var2.getItem() == OreSpawnMain.MyPeacock) && par1EntityPlayer.getDistanceSqToEntity(this) < 16.0D) {
			...
			return true;
		} else if (this.isTamed() && var2 != null && var2.getItem() == OreSpawnMain.MyRuby && par1EntityPlayer.getDistanceSqToEntity(this) < 16.0D && this.func_152114_e(par1EntityPlayer)) {
			if (!this.worldObj.isRemote) {
				this.voice_enable = 0;
				this.dataWatcher.updateObject(23, this.voice_enable);
				this.playTameEffect(true);
	...
	protected String getLivingSound() {
		if (!this.isSitting() && this.voice_enable != 0) {
			if (OreSpawnMain.bro_mode != 0 && this.worldObj.rand.nextInt(2) == 1) {
				return null;
			} else if (this.worldObj.rand.nextInt(11) == 1) {
				Entity victim = this.getAttackTarget();
				if (victim != null) {
					return null;
				} else if (!this.isInWater() && !this.handleLavaMovement()) {
					if (this.worldObj.rand.nextInt(4) != 0) {
						if (this.posY < (double)60.0F) {
							return null;
						}
					...
					if (this.isTamed()) {
						if ((float)this.mygetMaxHealth() > this.getHealth()) {
							...
							return OreSpawnMain.bro_mode != 0 ? "orespawn:bb_happy" : "orespawn:b_happy";
						}
					} else {
						return null;
					}
	...
	protected String getHurtSound() {
		if (this.voice_enable == 0) {
			...
			return OreSpawnMain.bro_mode != 0 && this.worldObj.rand.nextInt(2) == 1 ? null : "orespawn:b_ow";
		}
	}
