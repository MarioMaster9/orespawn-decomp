...
public class ModelPointysaurus extends ModelBase
{
	...
	public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5)
	{
		...
		this.head.rotateAngleX = (float)Math.toRadians((double)f4) * 0.45F;
		...
		if (e.getAttacking() != 0) {
			...
			newangle = MathHelper.cos(f2 * 0.3F * this.wingspeed) * (float)Math.PI * 0.05F;
		}
		this.tail.rotateAngleY = newangle;
		
		newangle = MathHelper.cos(f2 * 0.02F * this.wingspeed) * (float)Math.PI * 0.15F;
