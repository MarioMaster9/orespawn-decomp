...
public class ThePrince extends EntityTameable {
	...
	public boolean interact(EntityPlayer par1EntityPlayer) {
		...
		if (var2 != null && var2.getItem() == Item.getItemFromBlock(Blocks.diamond_block) && par1EntityPlayer.getDistanceSqToEntity(this) < 16.0D) {
			...
			return true;
		} else if (this.isTamed() && var2 != null && this.func_152114_e(par1EntityPlayer) && par1EntityPlayer.getDistanceSqToEntity(this) < 16.0D && var2.getItem() instanceof ItemFood) {
			if (!this.worldObj.isRemote) {
			...
			if (!this.worldObj.isRemote) {
				...
				healthMessage = String.format("Prince fireballs extinguished.");
			...
			if (!this.worldObj.isRemote) {
				...
				this.setSpyroFire(1);
				String healthMessage = new String();
				healthMessage = String.format("Prince fireballs lit!");
				par1EntityPlayer.addChatComponentMessage(new ChatComponentText(healthMessage));
			}
			...
			return true;
		} else if (var2 != null && var2.getItem() == Items.diamond && par1EntityPlayer.getDistanceSqToEntity(this) < 16.0D && this.func_152114_e(par1EntityPlayer) && !this.worldObj.isRemote && this.ok_to_grow != 0) {
			Entity ent = null;
	...
	protected String getLivingSound() {
		if (this.isSitting()) {
			...
			return this.getAttacking() == 0 ? null : "orespawn:roar";
		}
	}
	...
	protected void updateAITasks() {
		if (!this.isDead) {
			...
			if (this.worldObj.rand.nextInt(200) == 1 && this.getHealth() < (float)this.mygetMaxHealth()) {
				this.heal(1.0F);
			}
		...
		if (this.worldObj.rand.nextInt(7) == 1 && this.worldObj.difficultySetting != EnumDifficulty.PEACEFUL) {
			...
			if (e != null) {
				if (this.isTamed() && this.getHealth() / (float)this.mygetMaxHealth() < 0.25F) {
					...
					} else if (this.getDistanceSqToEntity(e) > 25.0D && this.getDistanceSqToEntity(e) < (double)144.0F && !this.isInWater() && this.getSpyroFire() != 0 && (this.worldObj.rand.nextInt(3) == 0 || this.worldObj.rand.nextInt(4) == 1)) {
	...
	private boolean isSuitableTarget(EntityLivingBase par1EntityLiving, boolean par2) {
		if (this.worldObj.difficultySetting == EnumDifficulty.PEACEFUL) {
			...
			return true;
		} else if (par1EntityLiving instanceof Dragonfly) {
			return true;
		} else {
