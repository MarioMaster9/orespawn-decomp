...
public class ModelHammerhead extends ModelBase
{
	...
	public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5)
	{
		...
		if ((double)f1 > 0.1) {
			...
			newangle2 = MathHelper.cos((float)((double)(f2 * 1.3F * this.wingspeed) + (Math.PI / 4D))) * (float)Math.PI * 0.1F * f1;
		} else {
			newangle = 0.0F;
		}
		...
		newangle = MathHelper.cos(f2 * 0.3F * this.wingspeed) * (float)Math.PI * 0.03F;
		this.back_armour_4.rotateAngleY = 0.349F + newangle;
		this.back_armour_4R.rotateAngleY = -0.349F - newangle;
		
		if (e.getAttacking() != 0) {
			newangle = MathHelper.cos(f2 * 1.3F * this.wingspeed) * (float)Math.PI * 0.13F;
		} else {
			newangle = 0.0F;
		}
