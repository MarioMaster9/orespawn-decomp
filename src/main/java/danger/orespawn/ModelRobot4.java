...
public class ModelRobot4 extends ModelBase
{
	...
	public ModelRobot4(float f1)
	{
		...
		this.setRotation(this.middleleftspinetip, (-(float)Math.PI / 4F), 0.0F, 0.0F);
		...
		this.setRotation(this.middlerightspinetip, (-(float)Math.PI / 4F), 0.0F, 0.0F);
	...
	public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5)
	{
		...
		if ((double)f1 > 0.1) {
			newangle = MathHelper.cos(f2 * 0.5F * this.wingspeed) * (float)Math.PI * 0.15F * f1;
		} else {
			newangle = 0.0F;
		}
		...
		this.head.rotateAngleY = (float)Math.toRadians((double)f3 / 1.5D);
		
		float amp = ((float)Math.PI / 4F);
		if (e.getAttacking() != 0) {
			newangle = MathHelper.cos((float)Math.toRadians((double)(f2 % 360.0F)) * this.wingspeed * 6.0F) * amp;
		...
		if ((double)newangle > (double)amp / 3.0D) {
		...
		double newposy = (double)((float)((double)this.leftsholder.rotationPointY + Math.cos((double)this.leftupperarm.rotateAngleX) * 14.0D));
		double newposz = (double)((float)((double)this.leftsholder.rotationPointZ + Math.sin((double)this.leftupperarm.rotateAngleX) * 14.0D));
