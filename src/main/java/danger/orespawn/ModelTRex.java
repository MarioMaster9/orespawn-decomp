...
public class ModelTRex extends ModelBase
{
	...
	public ModelTRex(float f1)
	{
		...
		this.setRotation(this.Shape11, (-(float)Math.PI / 6F), 0.0F, 0.0F);
		...
		this.setRotation(this.Shape17, (-(float)Math.PI / 6F), 0.0F, 0.0F);
	...
	public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5)
	{
		...
		if (e.getAttacking() != 0) {
			this.jaw.rotateAngleX = 0.52F + MathHelper.cos(f2 * 0.45F) * (float)Math.PI * 0.18F;
		} else {
